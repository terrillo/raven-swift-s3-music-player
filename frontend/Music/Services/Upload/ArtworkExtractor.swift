...
struct ArtworkExtractor {
    ...
    private func determineMimeType(from data: Data) -> String {
        ...
        // Ensure we have at least 4 bytes before checking
        ...
        if data.count >= 12 && bytes.count >= 4 && bytes[0] == 0x52 && bytes[1] == 0x49 && bytes[2] == 0x46 && bytes[3] == 0x46 {
            let webpBytes = Array(data[8..<12])
            if webpBytes.count >= 4 && webpBytes == [0x57, 0x45, 0x42, 0x50] {
                return "image/webp"
            }
        }
